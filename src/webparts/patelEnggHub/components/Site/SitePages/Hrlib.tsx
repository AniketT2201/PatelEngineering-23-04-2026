...
ISiteNameOps().getSiteNameData(props).then((SiteNameColl) => {
        console.log(' SiteNamelibrary Data received:', SiteNameColl);
            ...
            ISubMenuOps().getSubMenuData(props).then((SubMenuColl) => {
              ...
              const filteredSubMenu1 = SubMenuColl.filter(item => {
                ...
                if (item.GroupAccess && Array.isArray(item.GroupAccess)) {
                  ...
                  return item.GroupAccess.some(group => GroupDataTitle.includes(itemTitle));
                }
              ...
              //  alert('You are not Authorize for this page!!');
                ...
                // const filteredItems = filteredSubMenu1.filter(item =>
                   
                //     new RegExp(`\\b${itemTitle.toLowerCase().trim()}\\b`, 'i').test(item.Title.toLowerCase().trim())
                //   );
                ...
                // const filteredItems = filteredSubMenu1.filter(item =>
                //     // item.Title.toLowerCase().trim().includes(itemTitle.toLowerCase().trim())
              ...
              {
               for (var f=0;f<filteredSubMenu.length;f++){
                   if(filteredSubMenu[f].Title==itemTitle){
                      counter++;
    ...
    const FetchSubFolders = async (
        ...
        try {
            ...
            let FolderPath = record.ServerRelativeUrl.split("/MAM/");
            if (!FolderPath[1]) {
...
// //`${webAbsoluteUrl}/_api/web/lists/getbytitle('${'/sites/PatelEngineeringSafetyPortal/'+ parent.libraryName}')/items?$select=*,Name,FileLeafRef,FSObjType,ServerRelativeUrl,Id,Title&$expand=Folders,Files,Files/ModifiedBy`,
//                 { headers: { Accept: "application/json;odata=verbose" } }
//             );
...
//                     // Check file extension to decide the FileUrl
//                     let fileUrl = file.ServerRelativeUrl;
//                     const fileExtension = file.Name.split('.').pop()?.toLowerCase();
                
//                     if (fileExtension === 'pdf') {
//                         fileUrl = file.ServerRelativeUrl; // For PDF, use the existing server-relative URL
//                     } else if (fileExtension === 'docx') {
//                         fileUrl = file.ServerRelativeUrl +'?web=1';
//                       //  fileUrl = webAbsoluteUrl + file.ServerRelativeUrl; // For DOCX, prepend the webAbsoluteUrl
//                     }
...
//                         fileUrl = file.ServerRelativeUrl + '?web=1'; // For PPT/PPTX, open in a new page using the `web=1` parameter
//                     }
//                      else if (fileExtension === 'csv') {
//                                 fileUrl = file.ServerRelativeUrl + '?web=1'; // For CSV, open in a new page using the `web=1` parameter
//                             }
//                             else if (fileExtension === 'jpg' || fileExtension === 'jpeg' || fileExtension === 'png' || fileExtension === 'gif' || fileExtension === 'bmp') {
//                                 fileUrl = file.ServerRelativeUrl + '?web=1'; // For images, use the same logic if needed
//                             }
    ...
    try {
        ...
        if (data.d) {
            const folderList: FolderItem[] = await Promise.all(
                data.d.Folders.results.map(async (folder: any) => {
                    ...
                    return {
                        ...
                        ServerRedirectedEmbedUrl: folder.ServerRedirectedEmbedUrl,
                        Type: "Folder",
                        items: [], // Initialize empty, or fetch recursively if needed
                        LastModified: new Date(folder.TimeLastModified).toLocaleString(),
                        ModifiedBy: modifiedBy,
                    };
                })
            ...
            const fileList: FolderItem[] = data.d.Files.results.map((file: any) => {
                ...
                if (["pdf", "docx", "xlsx", "xls", "ppt", "pptx", "csv", "jpg", "jpeg", "png", "gif", "bmp"].includes(fileExtension)) {
                    fileUrl += '?web=1';
                }
            ...
            // Match file metadata from parent.items if available
            const matchedItems: FolderItem[] = fileList.map((file) => {
                const matchedItem = parent.items?.find((item: any) => item.Name === file.Name || item.FileName === file.Name);
                return {
                    ...file,
                    Category: matchedItem?.Category || file.Category || "",
                    Details: matchedItem?.Details || file.Details || "",
                };
            });

            // ✅ Set current items to both folders and matched files
            setCurrentItems([...folderList, ...matchedItems]);

            // ✅ Update breadcrumb (avoid duplicate entries)
            setBreadcrumb((prev) => {
                const alreadyExists = prev.some(b => b.ServerRelativeUrl === parent.ServerRelativeUrl);
                if (!alreadyExists) {
                    return [...prev, parent];
                }
                return prev;
...
const fetchModifiedBy = async (folderUrl: string) => {
    ...
    await fetchItems(selectedFolder);
};
    ...
    if (breadcrumb.length === 0) {
        ...
        currentFolder = breadcrumb[breadcrumb.length - 1].ServerRelativeUrl;
    }
    ...
    try {
        ...
        // Step 1: Get existing files in the folder
        const existingFilesResponse = await fetch(
            `${webAbsoluteUrl}/_api/web/GetFolderByServerRelativeUrl('${currentFolder}')/Files?$select=Name`,
            {
                method: "GET",
                headers: {
                    Accept: "application/json;odata=verbose",
        ...
        const existingFileNames = existingFilesData.d.results.map((file: any) => file.Name.toLowerCase());

        // Step 2: Detect duplicates (ensure each is only counted once)
        const duplicatesSet = new Set<string>();

        const uniqueFiles = fileList.filter(file => {
            const lowerName = file.name.toLowerCase();
            if (existingFileNames.includes(lowerName)) {
                duplicatesSet.add(file.name); // Add original casing
                return false;
            }
            return true;
        });
        ...
        // Step 3: Alert duplicates
        if (duplicates.length > 0) {
            alert(`Item already exist:\n${duplicates.join("\n")}`);
        }

        // Step 4: Upload non-duplicate files
        ...
        if (successFiles.length > 0) {
            ...
            breadcrumb.length > 0 ? fetchItems(breadcrumb[breadcrumb.length - 1]) : window.location.reload();
        }

    } catch (error) {
        message.error("Error occurred while uploading. Please try again.");
    ...
    //   // Replace with the desired library name dynamically if needed
    ...
    //         // Log the cleaned path and request digest for debugging
    //         // console.log('Cleaned Path:', cleanedPath);
    //         // console.log('Library Name Clean:', LibraryNameClean);
    //         // console.log('Request Digest:', requestDigest);
    
    //         // Fetch the document library to check if it's empty
    //         const checkLibraryResponse = await fetch(`${webAbsoluteUrl}/_api/web/lists/getbytitle('Documents')/items?$top=1`, {
    //             method: "GET",
    //             headers: {
    //                 Accept: "application/json;odata=verbose",
    //             },
    //         });
    //         const checkLibraryData = await checkLibraryResponse.json();
    //         const isLibraryEmpty = checkLibraryData.d.results.length === 0;
    
    //         console.log('Is Library Empty:', isLibraryEmpty); // Debugging the empty library check
    //         // var LibraryNameClean
    //         // if(isLibraryEmpty==true){
    //         //     const currentPath = itemTitle;
    //         //     const cleanedPath = currentPath.split("/").map(part => part.trim()).filter(Boolean).join("/");
    //         //   const fullFolderPath = `${cleanedPath}/${newFolderName.trim()}`;
    //         //    LibraryNameClean = fullFolderPath
    //         // }
    // //else{
    //             const currentPath = breadcrumb.length>0?breadcrumb[breadcrumb.length - 1].ServerRelativeUrl:itemTitle;
    //             const cleanedPath = currentPath.split("/").map(part => part.trim()).filter(Boolean).join("/");
    //             const fullFolderPath = `${cleanedPath}/${newFolderName.trim()}`;
    //             const LibraryNameClean = breadcrumb.length>0?fullFolderPath.split("/").slice(2).join("/"):fullFolderPath;
            
          
            
    //         if (isLibraryEmpty) {
    //             // Create folder inside the document library
    ...
    //                 },
    //                 body: JSON.stringify({
    //                     ServerRelativeUrl: LibraryNameClean, // Create folder inside document library
    //                 }),
    //             });
    
    //             // Log the response for debugging
    //             const responseData = await response.json();
    //             console.log('Folder Creation Response:', responseData); // Log response to check the error
    ...
    //                 //window.location.reload(); // Reload items in the library
    //             } else {
    //                 message.error(`Failed to create folder: ${responseData.error.message.value}`);
    //             }
    //         } else {
    //             message.info("Document library is not empty. Please ensure the library is empty before creating a folder.");
    //         }
    //     } catch (error) {
...
const handleCreateFolder = async () => {
    ...
    try {
        ...
        // ✅ FIX: Calculate siteRelativeUrl from webAbsoluteUrl
        const siteRelativeUrl = new URL(webAbsoluteUrl).pathname;

        // 2. Determine current folder path
        const currentFolderUrl = breadcrumb.length > 0
            ? breadcrumb[breadcrumb.length - 1].ServerRelativeUrl
            : `${siteRelativeUrl}/${itemTitle}`; // fallback to library root

        const fullFolderPath = `${currentFolderUrl}/${newFolderName.trim()}`.replace(/\/+/g, "/");

        console.log("Full Folder Path:", fullFolderPath);

        // 3. Check if folder already exists
        const encodedPath = encodeURIComponent(fullFolderPath);
        const folderExistsResponse = await fetch(`${webAbsoluteUrl}/_api/web/GetFolderByServerRelativeUrl('${encodedPath}')`, {
            method: "GET",
            headers: { Accept: "application/json;odata=verbose" }
        });

        if (folderExistsResponse.ok) {
            message.warning(`Folder '${newFolderName}' already exists!`);
            alert(`Folder '${newFolderName}' already exists!`);
            return;
        }

        // 4. Create the folder
        const createResponse = await fetch(`${webAbsoluteUrl}/_api/web/folders`, {
            method: "POST",
            headers: {
                Accept: "application/json;odata=verbose",
                "Content-Type": "application/json;odata=verbose",
            ...
            body: JSON.stringify({
                ...
                ServerRelativeUrl: fullFolderPath
            })
        });
        ...
        if (createResponse.ok) {
            ...
            const errorMessage = errorJson?.error?.message?.value || "Unknown error";
    ...
    const handleClick = (childItemTitle: string, e: React.MouseEvent) => {
    e.preventDefault(); // Prevent default link behavior
    var urltoredirect='Library/'+childItemTitle;
    history.push(`/${urltoredirect}`);
    window.location.reload();

    // Log the Title (or perform any action you want with childItem.Title)
    console.log("Clicked item title:", childItemTitle);

    // You can use the title in logic here (e.g., redirect, set state, etc.)
    // If you want to redirect manually:
    // window.location.href = childItem.Url; // For external links, use this
  };
...
// const  createBlankFile=async (type:any):Promise<void>=> {
//   let folderPath;
...
//     } else {
//         folderPath = breadcrumb[breadcrumb.length - 1].ServerRelativeUrl;
//     }
//   //const folderPath = this.state.currentFolderPath; // your selected folder
...
//       templateFile = require("../assets/templates/Document.docx");
...
//       templateFile = require("../assets/templates/Excel.xlsx");

//       break; 
 
//     case "ppt":

//       fileName = "New Presentation.pptx";

//       templateFile = require("../assets/templates/PPT.pptx");

//       break;

//   }   
 
//   const fileBuffer = await fetch(templateFile).then(r => r.arrayBuffer());
...
//   //const folderPath = this.state.currentFolderPath;
//   let templatePath = "";
//   let fileName = "";
 
//   switch (type) { 
//     case "word":
//       templatePath = "/sites/IT/TemplateFiles/Document.docx";
//       fileName = "Document.docx";
//       break;
 
//     case "excel":
//       templatePath = "/sites/IT/TemplateFiles/Book.xlsx";
//       fileName = "New Workbook.xlsx";
//       break;
 
//     case "ppt":
//       templatePath = "/sites/IT/TemplateFiles/Presentation.pptx";
//       fileName = "New Presentation.pptx";
//       break;
//   }
...
//     .getFolderByServerRelativePath(newUrl)
//     .files.addUsingPath(fileName, fileBuffer, { Overwrite: false });
 
//   const newFileUrl = uploadResult.data.ServerRelativeUrl;
 
//   // Open file
//   const openUrl = `${props.currentSPContext.pageContext.web.absoluteUrl}${newFileUrl}?web=1`;
...
const createBlankFile = async (type: any): Promise<void> => {
  ...
  if (breadcrumb.length === 0) {
    ...
    folderPath = `${cleanedPath}/${newFolderName.trim()}`;
  } else {
    folderPath = breadcrumb[breadcrumb.length - 1].ServerRelativeUrl;
  }
...
let newUrl = parts.join('/');
  ...
  try {
    ...
    // 1️⃣ Get template file buffer
    const fileBuffer = await sp.web
      .getFileByServerRelativePath(templatePath)
      .getBuffer();

    // 2️⃣ Upload into selected folder (NO parent trimming)
    const uploadResult = await sp.web
      .getFolderByServerRelativePath(newUrl)
      .files.addUsingPath(fileName, fileBuffer, { Overwrite: false });
                ...
                <div className="col-md-12 col-sm-11 w--8">
                    <div >
                        <div className="">
                            ...
                            <div className={`nav-btn  pl-10 libbox subnav-btn ${isOpensub ? 'open' : ''}`} id="navsubsection">
                                                                ...
                                                                {getsubChildren(item.Id).map((childItem) => (
                                                                    <li key={childItem.Id} className="dropdown-link"  >
                                                                        {childItem.Url?.startsWith("https") || childItem.Url?.startsWith("http") ? (
                                                                            ...
                                                                            // <Link to={childItem.Url || "#"} onClick={(e) => handleClick(childItem.Title, e)}>
                                                                            // {childItem.Title}
                                                                            // </Link>
                                                                             <a href='#' target="_blank" onClick={() => { openLinkInNewWindow(childItem.Url) }}>{childItem.Title}</a>
                                                                        ) : (  
                                                                        // <Link  to={childItem.Url || "#"} onClick={handleClick}>
                                                                        //     {childItem.Title}
                                                                        //     </Link>
                                                                        <Link to={childItem.Url || "#"} onClick={(e) => handleClick(childItem.Title, e)}>
                                                                        {childItem.Title}
            ...
            <div className='main-box mtl-50'>
                <div className='row'>
                                        ...
                                        <div key={index} className="LibraryFilesalign">
                                            <div
                                                className="foldersnames"
                                                    ...
                                                    const folderName = e.currentTarget.querySelector("span.folderName") as HTMLElement;
                                                    const toggleIcon = e.currentTarget.querySelector("span.toggleIcon i") as HTMLElement;

                                                    const isAlreadyVisible = filesElement?.classList.contains("visible");
                                                    ...
                                                    const parentElement = element.previousSibling as HTMLElement;
                                                    if (parentElement) {
                                                    const prevToggleIcon = parentElement.querySelector(".toggleIcon i") as HTMLElement;
                                                    const prevFolderName = parentElement.querySelector(".folderName") as HTMLElement;
                                                    if (prevToggleIcon) prevToggleIcon.className = "fa fa-plus";
                                                    if (prevFolderName) prevFolderName.style.color = "black";
                                                    }
                                                    });
                                                    ...
                                                    if (toggleIcon) toggleIcon.className = "fa fa-plus";
                                                    folderName.style.color = "black";

                                                    if (breadcrumb.length > 0) {
                                                    const filtervalues = breadcrumb.filter((item, index) => index === 0);
                                                    setBreadcrumb(filtervalues);
                                                    }
                                                    ...
                                                    if (toggleIcon) toggleIcon.className = "fa fa-minus";
                                            ...
                                            >
                                                ...
                                                <span style={{ fontSize: "13px", marginRight: "8px" }}><i className="fa fa-folder-o" aria-hidden="true"></i></span>
                                                    ...
                                                    Subfolders.map((item, index) => (
                                                        <div key={index} className="foldersnames">
                                                            <div
                                                                onClick={(e) => {
                                                                    ...
                                                                    if (filesElement?.classList.contains("visible")) {
                                                                            ...
                                                                            if (toggleIcon) toggleIcon.className = "fa fa-plus"; // 👍 works now
                                                                            folderName.style.color = "black";
                                                                        } else if (filesElement) {
                                                                            filesElement.classList.remove("hidden");
                                                                            filesElement.classList.add("visible");

                                                                            if (toggleIcon) toggleIcon.className = "fa fa-minus"; // 👍 works now
                                                                            folderName.style.color = "red";
                                                                            fetchItems(item);
                                                                        }
                                                            ...
                                                            >
                                                                ...
                                                                <span style={{ fontSize: "15px", marginRight: "8px" }}><i className="fa fa-folder-o" aria-hidden="true"></i></span>
                                                                <span className="folderName" style={{ fontSize: "13px" }}
                                                                >{item.Name}</span>
                                                            </div>
                                                                        ...
                                                                        <div key={subIndex} className="foldersnames">
                                                                            <div
                                                                                onClick={(e) => {
                                                                                    ...
                                                                                    const subToggleIcon = e.currentTarget.querySelector("span.toggleIcon i") as HTMLElement;
                                                                                    ...
                                                                                    if (subFilesElement?.classList.contains("visible")) {
                                                                                        ...
                                                                                        if (subToggleIcon) subToggleIcon.className = "fa fa-plus"; // 👍 works now
                                                                                        subFilesElement.style.color = "black";
                                                                                    } else if (subFilesElement) {
                                                                                        subFilesElement.classList.remove("hidden");
                                                                                        subFilesElement.classList.add("visible");

                                                                                        if (subToggleIcon) subToggleIcon.className = "fa fa-minus"; // 👍 works now
                                                                                        subFilesElement.style.color = "red";
                                                                                        fetchItems(item);
                                                                                    }
                                                                                    ...
                                                                                    //     if (subToggleIcon) subToggleIcon.className = "fa fa-plus";
                                                                                    //     subFolderName.style.color = "black";
                                                                                    // } else if (subFilesElement) {
                                                                                    //     subFilesElement.classList.add("visible");
                                                                                    //     // subToggleIcon.textContent = "-";
                                                                                    //     if (subToggleIcon) subToggleIcon.className = "fa fa-minus";
                    ...
                    <div className='col-md-8 col-sm-12 pr-2'>
                        <div className=''>
                            <div className='' style={{ boxShadow: '0 0 8px 0 #b3b3b3' }}>
                                <div className="">
                                    <div className="col-md-12 topheader" style={{paddingTop:'15px'}}>
                                        <div className="">

                                            <div className="breadcrumbutton-text">
                                                <Link className="homebutton" to={'/'} >Home</Link>
                                                {breadcrumb.length === 0 ? " Libraries" : breadcrumb.map((item, index) => (
                                                    <span key={index} onClick={() => navigateBack(index)} style={{ cursor: "pointer" }}>
                                                        {/* {" > " + item.Name} */} <span >{">"}</span>
                                        ...
                                        <div className="blockLibrary  pt-10">
                                            <div className="blockLibrary-head">
                                                ...
                                                <div >
                                                    ...
                                                    disableAnimation
                                                /> */}
                                                    <span className="Uploadbox opendocumentbox">
                                                        {/* <Button icon={<PlusOutlined />} onClick={openModal} type="primary" className="UploadButton uploadbtn "></Button> */}
                                                        <Button icon={<PlusOutlined />}  type="primary" className="UploadButton uploadbtn ">
                                                            New 
                                                        </Button>
                                                        <div className="documentbox">
                                                            <ul>
                                                                <li onClick={() =>createBlankFile("word")}>
                                                                    <img src=""  />document 
                                                                </li>
                                                                 <li>
                                                                    <img src=""/>ppt
                                                                    ...
                                                                    render: (text, record) => (
                                                                        <span 
                                                                                ...
                                                                                onClick={() => {
                                                                                    ...
                                                                                    if (fileExtension === 'docx' || fileExtension === 'xls' || fileExtension === 'pdf'|| fileExtension === 'xlsx'
                                                                                       || fileExtension === 'ppt' || fileExtension === 'pptx' ||fileExtension === 'csv' || fileExtension === 'jpg' || fileExtension === 'jpeg' || fileExtension === 'png' || fileExtension === 'gif' || fileExtension === 'bmp'
                                                                                    ) {
