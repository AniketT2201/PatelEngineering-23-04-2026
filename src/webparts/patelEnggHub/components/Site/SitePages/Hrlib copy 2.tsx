    ...
    const fetchLibraries1 = async (libraryName: any) => {
        try {
            ...
            const responseCategory = await fetch(
                `${webAbsoluteUrl}/_api/web/lists/getbytitle('${libraryName}')/items?$select=*,Editor/Title,File/Name&$expand=Editor,File`,
                { headers: { Accept: "application/json;odata=verbose" } }
            );
    ...
    const fetchItems = async (parent: FolderItem) => {
        try {
            const response = await fetch(
                ...
                `${webAbsoluteUrl}/_api/web/GetFolderByServerRelativeUrl('${parent.ServerRelativeUrl}')?$select=*,Name,ServerRedirectedEmbedUrl,ServerRelativeUrl,Id,Title&$expand=Folders,Files,Files/ModifiedBy`,
            ...
            if (data.d) {
                ...
                const fileList: FolderItem[] = data.d.Files.results.map((file: any) => ({
                    ...
                    //FileUrl: webAbsoluteUrl + file.ServerRelativeUrl,
                    FileUrl: file.ServerRedirectedEmbedUrl,
                    Type: "File",
            ...
            const fetchFilesRecursive = async (folderUrl: string) => {
                try {
                    ...
                    const fileResponse = await fetch(
                        //`${webAbsoluteUrl}/_api/web/GetFolderByServerRelativeUrl('${folderUrl}')/Files?$select=Name,ServerRelativeUrl,ServerRedirectedEmbedUri,TimeLastModified,Author/Title&$expand=Author`,
                        `${webAbsoluteUrl}/_api/web/GetFolderByServerRelativeUrl('${folderUrl}')/Files?$select=*,Name,ServerRelativeUrl,TimeLastModified,Author/Title,Author/Id,Author/Email,Author/TimeZone,FileLeafRef,FSObjType,File_x0020_Type,File_x0020_Size,UniqueId,ServerRedirectedEmbedUri&$expand=Author`,
                        { headers: { Accept: "application/json;odata=verbose" } }
                    );
                    ...
                    if (fileResponse.ok) {
                        ...
                        const files = fileData.d.results.map((file: any) => ({
                            Name: file.Name,
                            ServerRelativeUrl: file.ServerRelativeUrl,
                            ServerRedirectedEmbedUri: file.ServerRedirectedEmbedUri,
                           // FileUrl: `${webAbsoluteUrl}${file.ServerRelativeUrl}`,
                            FileUrl: `${webAbsoluteUrl}${file.ServerRedirectedEmbedUri}`,
